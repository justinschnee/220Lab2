...
void PrintAdsVal();
void PrintAdsVal2(int x);
int functThree();
void functFour(int *y);
void RanNum(int &z);
void PointFun(int *a, int *b);
void CallBy(char *c, char &d, char e, char *f, char &g);
void LargestVal(int len, int *add);
void functNine(int len, int &nineOne, int &nineTwo);
void ReverseArray(int tenOne);
void AscendArray(int elevenOne, int elevenTwo);
...
void ReverseArray(int ary1) {
	int ReverseArray[ary1];
	for (int i = 0; i < ary1; i++) {
		ReverseArray[i] = rand() % 50;
	}
	for (int j = 0; j < ary1; j++) {
		cout << ReverseArray[j] << ", ";
	}
	cout << endl;
}
